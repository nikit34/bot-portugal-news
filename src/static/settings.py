...
# Аудиторный фильтр: аудитория страницы на 74,5% португальская, а фиды несут ещё и
# бразильский чемпионат — отсекаем посты с бразильским домашним сигналом и без
# португальского (см. audience_filter.py). По умолчанию ВЫКЛ; включается env.
AUDIENCE_FILTER_ENABLED = os.getenv('AUDIENCE_FILTER_ENABLED', 'false').lower() in ('1', 'true', 'yes')
...
# Бонус кандидату с португальским сигналом (см. audience_filter). Аудиторный гейт
# только УБИРАЕТ бразильское домашнее, но не поднимает португальское, поэтому слот
# может занять международный сюжет без португальского угла. 0 => не поднимать.
RANKER_PT_BONUS = float(os.getenv('RANKER_PT_BONUS', '0.0'))
...
REDIS_DEDUP_MAX_HEADS = int(os.getenv('REDIS_DEDUP_MAX_HEADS', str(COUNT_UNIQUE_MESSAGES * 3)))

DEDUP_ALLOW_EMPTY_LEDGER = _flag('DEDUP_ALLOW_EMPTY_LEDGER', 'false')
...
RANKER_SOURCE_SHARE = float(os.getenv('RANKER_SOURCE_SHARE', '0.34'))
