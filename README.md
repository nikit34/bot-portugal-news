...
постили», и прогон переопубликовал бы всю ленту в три канала. Поэтому бот в такой
...
## Telegram: один аккаунт, без бота

Раньше в Telegram ходили двое: бот-аккаунт (@PostmanPortugalBot) публиковал в
канал через Telethon и слал служебные сообщения в debug-чат по HTTP Bot API, а
юзер-аккаунт читал источники. Бота больше нет - всё делает один юзер-аккаунт из
секрета `TELEGRAM_SESSION`.

Почему так: публикация в канал зависела от того, что бот числится в
администраторах. Стоило правам пропасть, и посты переставали уходить, а прогон
сжигал по пять повторных попыток на каждую новость. Юзер-аккаунт - владелец
канала, у него это отвалиться не может.

Что из этого следует:

* Секрет `TELEGRAM_TOKEN_BOT` больше не используется, его можно удалить.
* Аккаунт из `TELEGRAM_SESSION` должен состоять в debug-чате
  (`self.telegram_debug_chat_id`), иначе служебные сообщения молча не дойдут -
  отправка best-effort и прогон не роняет.
* Уведомление о падении самого job'а шлёт `tools/notify_debug.py` тем же
  аккаунтом: в YAML curl'ом это делать больше нечем.
