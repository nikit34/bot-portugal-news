...
и из них собирался дедуп-леджер. Площадка была базой. Теперь база - Redis, а
истории площадок остались страховкой.

Включается одним секретом. Без `REDIS_URL` бот работает ровно как раньше:
...
| `botnews:<config>:published:synced_at` | string | когда последний раз сверялись с площадками |
| `botnews:<config>:candidates` | zset | кандидаты, не добранные прошлым прогоном |

**Дедуп.** Леджер грузится из Redis, и чтение трёх историй на старте пропускается.
...
**Деградация.** Redis недоступен или упал посреди прогона - он гасится до конца
прогона, а бот возвращается к чтению историй площадок. Флаш базы не приводит к
массовому репосту: пустой леджер = холодный старт = сидирование из истории площадок.
Плюс раз в `REDIS_DEDUP_RESYNC_SECONDS` (сутки) сверка с площадками идёт всё равно.

Ручки (все через env, откат без правки кода):

| Переменная | Дефолт | Что делает |
|---|---|---|
| `REDIS_URL` | пусто | адрес Redis; пусто = вся интеграция выключена |
...
| `REDIS_DEDUP_RESYNC_SECONDS` | 24 часа | как часто сверяться с площадками |
