...
ALL_ON = {Platform.FACEBOOK: True, Platform.INSTAGRAM: True}
...
def test_find_posted_fuzzy_match():
    posted = deque([['Original test message', {Platform.INSTAGRAM}]])
...
def test_decisions_already_on_all_is_duplicate():
    posted = deque([['x', {Platform.FACEBOOK, Platform.INSTAGRAM}]])
...
def test_decisions_backfills_only_missing_instagram():
    posted = deque([['x', {Platform.FACEBOOK}]])
    decision = get_decisions_publish_platforms('x', posted, ALL_ON)
...
def test_decisions_respect_disabled_platform():
    platforms = {Platform.FACEBOOK: False, Platform.INSTAGRAM: True}
...
def test_mark_posted_new_head_creates_entry():
    posted = deque()
    mark_posted(posted, 'x', {Platform.FACEBOOK, Platform.INSTAGRAM})
    assert _find_posted('x', posted)[1] == {Platform.FACEBOOK, Platform.INSTAGRAM}


def test_mark_posted_extends_existing_entry_without_duplicating():
    posted = deque([['x', {Platform.INSTAGRAM}]])
...
def test_instagram_not_reposted_after_facebook_failure():
    # Regression for the IG-dedup gap: FB failed but IG succeeded this run.
    # Next run must request only FB (the platform that failed), NOT re-post to IG.
    posted = process_post_histories([], [])
    mark_posted(posted, 'story', {Platform.INSTAGRAM})
...
def test_is_duplicate_publish_all_false():
    assert is_duplicate_publish(
        {Platform.FACEBOOK: False, Platform.INSTAGRAM: False})


def test_is_duplicate_publish_some_true():
    assert not is_duplicate_publish(
        {Platform.FACEBOOK: True, Platform.INSTAGRAM: False})


def test_is_duplicate_publish_all_true():
    assert not is_duplicate_publish(
        {Platform.FACEBOOK: True, Platform.INSTAGRAM: True})
